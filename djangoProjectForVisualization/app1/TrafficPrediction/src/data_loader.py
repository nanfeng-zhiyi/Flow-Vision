...
class PEMSDataset(Dataset):
    def __init__(self, data_path, num_nodes, divide_days, time_interval, history_length, train_mode):
        ...
        :param data_path: ["graph file name" , "flow dataset file name"], path to save the dataset file names
        :param num_nodes: number of nodes in graph
        :param divide_days: [ days of train dataset, days of test dataset], list to divide the original dataset
        :param time_interval: time interval between two traffic dataset records (mins)
        :param history_length: length of history dataset to be used
    ...
    def slice_data(data, history_length, index, train_mode):
        """
        :param data: np.array, normalized traffic dataset.
        :param history_length: int, length of history dataset to be used.
    ...
    def pre_process_data(data, norm_dim):
        """
        :param data: np.array, original traffic dataset without normalization.
        :param norm_dim: int, normalization dimension.
        :return:
            norm_base: list, [max_data, min_data], dataset of normalization base.
            norm_data: np.array, normalized traffic dataset.
        """
        norm_base = PEMSDataset.normalize_base(data, norm_dim)  # find the normalize base
        norm_data = PEMSDataset.normalize_data(norm_base[0], norm_base[1], data)  # normalize dataset
    ...
    def normalize_data(max_data, min_data, data):
        """
        :param max_data: np.array, max dataset.
        :param min_data: np.array, min dataset.
        :param data: np.array, original traffic dataset without normalization.
        :return:
            np.array, normalized traffic dataset.
        """
        mid = min_data
    ...
    def recover_data(max_data, min_data, data):
        ...
        :param max_data: np.array, max dataset.
        :param min_data: np.array, min dataset.
        :param data: np.array, normalized dataset.
        :return:
            recovered_data: np.array, recovered dataset.
        """
        mid = min_data
        base = max_data - min_data
...
def get_loader(ds_name="PEMS04"):
    ...
    train_data = PEMSDataset(data_path=["app1/TrafficPrediction/dataset/PEMS/{}/distance.csv".format(ds_name), "app1/TrafficPrediction/dataset/PEMS/{}/dataset.npz".format(ds_name)], num_nodes=num_nodes,
                             divide_days=[45, 14],
    ...
    test_data = PEMSDataset(data_path=["app1/TrafficPrediction/dataset/PEMS/{}/distance.csv".format(ds_name), "app1/TrafficPrediction/dataset/PEMS/{}/dataset.npz".format(ds_name)], num_nodes=num_nodes,
                            divide_days=[45, 14],
                            time_interval=5, history_length=6,
                            train_mode="test")
...
# 用于加载前端发送过来的数据文件
def vue_get_loader(dsname='data'):
    data_npz = np.load('E:/vue-data/data.npz')  # 直接加载
    data = data_npz['data']
    # print(data)
    node_num = data.shape[1]  # 图示中的道路节点数量
    # print(data.shape[1])
    test_data = PEMSDataset(data_path=["E:/vue-data/data.csv", "E:/vue-data/data.npz"], num_nodes=node_num,
                            divide_days=[45,14],
                            time_interval=5, history_length=6,
                            train_mode="test")
    test_loader = DataLoader(test_data, batch_size=64,shuffle=False)
...
def check_file_existence(dirname: str, filename: str):
