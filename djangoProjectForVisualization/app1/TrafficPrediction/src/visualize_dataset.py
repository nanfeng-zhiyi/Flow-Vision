...
def show_pred(test_loader, all_y_true, all_predict_values):
    ...
    plt.legend()
    plt.savefig("media/the first day pred flow in node {}.png".format(str(node_id)), dpi=400)
    ...
    plt.legend()
    plt.savefig("media/the first two weeks pred flow in node {}.png".format(str(node_id)), dpi=400)
...
if __name__ == '__main__':
    traffic_data = get_flow('app1/TrafficPrediction/dataset/PEMS/PEMS04/dataset.npz')
    print("dataset size {}".format(traffic_data.shape))
    # 采样某个结点的数据
    node_id = 224
    ...
    plt.legend(loc=0)
    plt.savefig("app1/TrafficPrediction/assets/vis.png")
    plt.show()
